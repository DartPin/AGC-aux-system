...
import axios from 'axios';
    ...
    mounted(){
        ...
        axios.get('http://localhost:8000/Quality/Department/' + this.$route.params.depart + '/QRCI/' + this.$route.params.id)
            .then(response => (
    ...
    methods:{
        saveQRCI(){
            ...
            axios.post('http://localhost:8000/Quality/Department/' + this.$route.params.depart + '/QRCI/' + this.$route.params.id, this.qrciData)
        },
        pushNewRow(){
            if (this.newRow.id != ''){
            ...
            this.qrciData.qrciRows.forEach(element =>{
                element.photos.forEach(photo =>{
                    let str = photo.title.split('.')
                      arr.push('http://localhost:8000/getImage/'+str[0])
                  })
            })
            console.log(arr)
            return arr
